...
public class GameControllerScript : MonoBehaviour
{
	...
	private void Update()
	{
		if (!learningActive)
		{
			...
			if ((Input.GetAxis("ItemSelect") > 0f || Input.GetButtonDown("ItemSelectLeft")) && Time.timeScale != 0f)
			{
				DecreaseItemSelection();
			}
			else if ((Input.GetAxis("ItemSelect") < 0f || Input.GetButtonDown("ItemSelectRight")) && Time.timeScale != 0f)
			{
